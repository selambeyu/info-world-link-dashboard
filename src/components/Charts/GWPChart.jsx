...
import data from "../../data/GWP.json";
// import NoDataAvailable from "src/components/NoDataAvailable";

export const OverviewGWPByYear = () => {
  ...
  useEffect(() => {
    if (data && data.length > 0) {
      const labels = data.map((status) => status.year_month);
      const series = data.map((status) => status.GWP_birr);
